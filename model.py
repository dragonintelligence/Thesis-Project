...
wandb.init(project="model_sentiment")
...
for epoch in range(wandb.config.epochs):
    ...
    for step in range(max(len(train_dataloader), len(train_dataloader_imdb))):
        ...
        model.zero_grad()
        outputs_story = model(input_ids=story_input_ids, attention_mask=story_attention_mask, labels=story_labels, task='story')
        ...
        model.zero_grad()
        outputs_sentiment = model(input_ids=sentiment_input_ids, attention_mask=sentiment_attention_mask, labels=sentiment_labels, task='sentiment')
    ...
    with torch.no_grad():
        for val_batch in valid_dataloader:
            ...
            outputs_val = model(input_ids=val_input_ids, attention_mask=val_attention_mask, labels=val_labels, task='story')
